...
def call_dot(infile, outpath):
    ...
    try:
        p = subprocess.Popen([
            'dot', '-Nfontsize=10', '-Efontsize=10', '-Efontname=Courier New',
            '-Tsvg', '-o', outfile, infile
        ])
        code = p.wait()
        ...
        if e.errno == 2:
            sys.exit(
                'Error: Could not call "dot" from GraphViz to create graph, please install it\n({}).'.
                format(e))
        else:
            sys.exit(
                'Error: Could not call "dot" from GraphViz to create graph\n({}).'.
                format(e))

    if code != 0:
        sys.exit(
            'Error: Could not call "dot" from to create graph {0} (return code {1}).'.
            format(outfile, code))
...
def generate_report(cpa_output_dir, functions, arg_path, report_path):
    ...
    try:
        with open(os.devnull, 'w') as devnull:
            subprocess.Popen(
                ['xdg-open', report_path], stdout=devnull, stderr=devnull)
            ...
            with open(cfa_path) as cfa_file:
                for line in cfa_file:
                    if start:
                        line = line.replace(
                            'class="node"',
                            'class="node" ng-dblclick="clickedCFAElement($event)"'
                        )
                        line = line.replace(
                            'class="edge"',
                            'class="edge" ng-dblclick="clickedCFAElement($event)"'
                        )
                        report.write(line)
                    if '<svg' in line:
                        report.write(line[:5] +
                                     " ng-show = \"cfaFunctionIsSet(" + str(i) +
                                     ")\" " + line[5:])
                        start = True
        i = i + 1
        ...
        with open(arg_path) as argfile:
            for line in argfile:
                ...
                if start:
                    line = line.replace(
                        'class="node"',
                        'class="node" ng-dblclick="clickedARGElement($event)"')
                    line = line.replace(
                        'class="edge"',
                        'class="edge" ng-dblclick="clickedARGElement($event)"')
                    report.write(line)
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=
        "Generate a HTML report with graphs from the CPAchecker output.")
    parser.add_argument(
        "-c",
        "--config",
        dest="configfile",
...
def read_CPAchecker_config(options):
    ...
    try:
        ...
        if e.errno:
            sys.exit(
                'Could not find output of CPAchecker in {}. Please specify correctpath with option --config\n({}).'.
    ...
    if not config.get('analysis.programNames'):
        sys.exit(
            'CPAchecker output does not specify path to analyzed program. Cannot generate report.'
        )
...
def main():
    ...
    arg_path = os.path.join(cpa_output_dir, config.get('cpa.arg.file',
                                                       'ARG.dot'))
    counter_example_path_template = os.path.join(
        cpa_output_dir,
        config.get('counterexample.export.report', 'Counterexample.%d.html'))
    report_path = os.path.join(cpa_output_dir,
                               config.get('report.file', 'Report.html'))

    counter_example_paths = glob.glob(
        counter_example_path_template.replace('%d', '*'))
    ...
    functions = [
        x[5:-4]
        for x in os.listdir(cpa_output_dir)
        if x.startswith('cfa__') and x.endswith('.dot')
    ]
    sorted_functions = sorted(functions)
    for function in sorted_functions:
        print("\t" + function)
        call_dot(
            os.path.join(cpa_output_dir, 'cfa__' + function + '.dot'),
            cpa_output_dir)

    for counter_example_path in counter_example_paths:
        generate_report(cpa_output_dir, sorted_functions, arg_path,
                        counter_example_path)
