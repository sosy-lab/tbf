...
def getBinary():
    ...
    if currentPlatform == 'Windows':
        ultimateBin = [
            'java', '-Xmx12G', '-Xms1G', '-jar',
            'plugins/org.eclipse.equinox.launcher_1.3.100.v20150511-1540.jar',
            '-data', '@user.home/.ultimate'
        ]
    else:
        ultimateBin = [
            'java', '-Xmx12G', '-Xms1G', '-jar',
...
def runUltimate(ultimateCall, terminationMode):
    ...
    try:
        ultimateProcess = subprocess.Popen(
            ultimateCall,
    ...
    while True:
        ...
        if (ultimateProcess.returncode != None and not line):
            if (ultimateProcess.returncode == 0):
                ...
                print('\nExecution finished with exit code ' +
                      str(ultimateProcess.returncode))
            break
        ...
        if (terminationMode):
            ...
            if (line.find(safetyString) != -1 or
                    line.find(allSpecString) != -1):
                safetyResult = 'TRUE'
...
def flatten(l):
    for el in l:
        if isinstance(el, list) and not isinstance(
                el, basestring) and not isinstance(el, (str, bytes)):
...
def createWitnessPassthroughArgumentsList(printWitness, prop, architecture,
                                          cFile):
    if not printWitness:
        return []
    ret = []
    ...
    try:
        sha = subprocess.Popen(
            ['sha1sum', cFile], stdout=subprocess.PIPE).communicate()[0]
...
def parseArgs():
    ...
    parser = argparse.ArgumentParser(
        description='Ultimate wrapper script for SVCOMP')
    parser.add_argument(
        '--version',
        action='store_true',
    ...
    parser.add_argument(
        ...
        metavar='<file>',
        help=
        'Activate witness validation mode (if supported) and specify a .graphml file as witness'
    )
    parser.add_argument(
        'spec', nargs=1, help='An property (.prp) file from SVCOMP')
    parser.add_argument(
        'architecture',
        choices=['32bit', '64bit'],
        help=
    ...
    if args.validate and witness == None:
        ...
        sys.exit(1)
    if args.validate:
        return propertyFileName, args.architecture, [
            args.file[0], witness
        ], args.full_output, args.validate
    else:
        return propertyFileName, args.architecture, args.file[
            0], args.full_output, args.validate


def createSettingsSearchString(memDeref, memDerefMemtrack, terminationMode,
                               overflowMode, architecture):
...
def getToolchainPath(termmode, memDeref, memDerefMemtrack, overflowMode,
                     witnessmode):
    searchString = None
    if termmode:
...
def main():
    ...
    terminationMode, memDeref, memDerefMemtrack, overflowMode = determineMode(
        propertyFileName)

    propFileStr = open(propertyFileName, 'r').read()

    toolchain = getToolchainPath(terminationMode, memDeref, memDerefMemtrack,
                                 overflowMode, validateWitness)
    settingsSearchString = createSettingsSearchString(
        memDeref, memDerefMemtrack, terminationMode, overflowMode, architecture)
    ...
    witnessPassthroughArguments = createWitnessPassthroughArgumentsList(
        not validateWitness and not terminationMode, propFileStr, architecture,
        inputFiles)

    # execute ultimate
    print('Version ' + version)
    ultimateCall = createUltimateCall(ultimateBin, [
        '-tc', toolchain, '-i', inputFiles, '-s', settingsArgument,
        witnessPassthroughArguments
    ])

    # actually run Ultimate
    safetyResult, memResult, overflow, overapprox, ultimateOutput, errorPath = runUltimate(
        ultimateCall, terminationMode)
    ...
    if (overapprox):
        ...
        safetyResult, memResult, overflow, overapprox, ultimate2Output, errorPath = runUltimate(
            ultimateCall, terminationMode)
    ...
    if safetyResult.startswith('FALSE'):
        print('Writing human readable error path to file {}'.format(
            errorPathFileName))
