...
def _create_argument_parser():
    ...
    parser.add_argument(
        ...
        default="**",
        help=
        "Only files matching this glob pattern are transported back to the client."
    )
...
def _init(config):
    ...
    logging.info('Using %s version %s.', webclient.tool_name(),
                 webclient.tool_revision())
    return webclient
...
def _parse_cpachecker_args(cpachecker_args):
    ...
    while True:
        try:
            ...
            if option in [
                    "-heap", "-timelimit", "-entryfunction", "-spec", "-config",
                    "-setprop"
            ]:
