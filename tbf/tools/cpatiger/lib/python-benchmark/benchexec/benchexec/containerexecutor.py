...
def add_basic_container_args(argument_parser):
    ...
    argument_parser.add_argument(
        ...
        action="store_false",
        help=
        "do not use a special minimal configuration for local user and host lookups inside the container"
    )
    argument_parser.add_argument(
        "--keep-tmp",
        action="store_true",
        help=
        "do not use a private /tmp for process (same as '--full-access-dir /tmp')"
    )
    argument_parser.add_argument(
        "--hidden-dir",
        metavar="DIR",
    ...
    argument_parser.add_argument(
        ...
        default=[],
        help=
        "give full access (read/write) to this host directory to processes inside container"
    )
...
def handle_basic_container_args(options, parser=None):
    ...
    def handle_dir_mode(path, mode):
        ...
        if path in dir_modes:
            error_fn("Cannot specify multiple directory modes for '{}'.".format(
                path))
        dir_modes[path] = mode
    ...
    if options.container_system_config:
        if dir_modes.get("/etc", dir_modes["/"]) != DIR_OVERLAY:
            logging.warning(
                "Specified directory mode for /etc implies --keep-system-config, "
        ...
        if not "/run/resolvconf" in dir_modes and os.path.isdir(
                "/run/resolvconf"):
...
def add_container_output_args(argument_parser):
    ...
    argument_parser.add_argument(
        ...
        default=[],
        help=
        "pattern for specifying which result files should be copied to the output directory "
        "(default: '.')")
...
def handle_container_output_args(options, parser):
    ...
    if options.result_files:
        result_files_patterns = [
            os.path.normpath(p) for p in options.result_files if p
        ]
...
def main(argv=None):
    ...
    executor = ContainerExecutor(
        uid=options.uid, gid=options.gid, **container_options)
    ...
    try:
        ...
        if options.debug:
            logging.exception(e)
        sys.exit("Cannot execute {0}: {1}.".format(
...
class ContainerExecutor(baseexecutor.BaseExecutor):
    ...
    def __init__(self,
        ...
        self._uid = (uid if uid is not None else container.CONTAINER_UID
                     if container_system_config else os.getuid())
        self._gid = (gid if gid is not None else container.CONTAINER_GID
                     if container_system_config else os.getgid())
        ...
        if container_system_config:
            if dir_modes.get("/etc", dir_modes.get("/")) != DIR_OVERLAY:
                raise ValueError(
                    "Cannot setup minimal system configuration for the container "
        ...
        for path, kind in dir_modes.items():
            ...
            if not os.path.isabs(path):
                raise ValueError(
                    "Invalid non-absolute directory '{}'.".format(path))
            if path == "/proc":
    ...
    def _get_result_files_base(self, temp_dir):
        ...
        if not self._use_namespaces:
            return super(ContainerExecutor,
    ...
    def _start_execution(self,
                         ...
                         **kwargs):
        if not self._use_namespaces:
            return super(ContainerExecutor, self)._start_execution(
                *args, **kwargs)
        else:
            if result_files_patterns:
                ...
                for pattern in result_files_patterns:
                    if not pattern:
                        raise ValueError(
                            "Invalid empty result-files pattern in {}"
                            .format(result_files_patterns))

                    pattern = os.path.normpath(pattern)
                    if pattern.startswith(".."):
                        raise ValueError(
                            "Invalid relative result-files pattern '{}'."
                            .format(pattern))
    ...
    def _start_execution_in_container(self, args, stdin, stdout, stderr, env,
        ...
        def child():
            ...
            try:
                ...
                if _HAS_SIGWAIT:
                    ...
                    container.forward_all_signals_async(grandchild_proc.pid,
                                                        args[0])
        ...
        try:  # parent
            ...
            container.setup_user_mapping(
                child_pid, uid=self._uid, gid=self._gid)
    ...
    def _setup_container_filesystem(self, temp_dir):
        ...
        def find_mode_for_dir(path, fstype):
            ...
            if result_mode == DIR_OVERLAY and (_is_below(path, b"/dev") or
                                               _is_below(path, b"/sys") or
                                               fstype == b"cgroup"):
                # Overlay does not make sense for /dev, /sys, and all cgroups.
                return DIR_READ_ONLY

            if result_mode == DIR_OVERLAY and (fstype == b"autofs" or
                                               fstype == b"vfat" or
                                               fstype == b"ntfs"):
        ...
        container.make_bind_mount(
            b"/", mount_base, recursive=True, private=True)
        ...
        for special_dir in self._dir_modes.keys():
            ...
            try:
                ...
                logging.debug("Failed to make %s a bind mount: %s", mount_path,
                              e)
    ...
    def _transfer_output_files(self, temp_dir, working_dir, output_dir,
                               patterns):
