...
class KillProcessOnOomThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            _ = os.read(
                self._efd,
                8)  # blocks and returns event number (we do not need it)
            ...
            if not self._finished.is_set():
                ...
                with open(os.path.join(self._cgroups[MEMORY], 'tasks'),
                          'rt') as tasks:
                    for task in tasks:
        ...
        if self._cgroups.has_value(MEMORY, limitFile):
            try:
                ...
                self._cgroups.set_value(
                    MEMORY, limitFile,
                    str(1 * _BYTE_FACTOR * _BYTE_FACTOR * _BYTE_FACTOR *
                        _BYTE_FACTOR * _BYTE_FACTOR))
            except IOError as e:
                logging.warning(
