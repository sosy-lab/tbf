...
benchmark_test_file = os.path.join(base_dir, 'doc',
                                   'benchmark-example-rand.xml')
benchmark_test_tasks = [
    'DTD files', 'Markdown files', 'XML files', 'Dummy tasks'
]
...
class BenchExecIntegrationTests(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.mkdtemp(
            prefix="BenchExec.benchexec.integration_test")
    ...
    def run_cmd(self, *args):
        try:
            output = subprocess.check_output(
                args=args, stderr=subprocess.STDOUT).decode()
    ...
    def run_benchexec_and_compare_expected_files(self,
                                                 *args,
                                                 name=None,
        ...
        self.run_cmd(*[
            ...
            '--startTime',
            '2015-01-01 00:00',
        ] + ([] if compress else ['--no-compress-results']) + list(args))
        ...
        if rundefs is None:
            ...
            expected_files += [
                'results.' + rundef + xml_suffix for rundef in rundefs
            ]
        ...
        if compress:
            with zipfile.ZipFile(
                    os.path.join(self.tmp,
                                 basename + "logfiles.zip")) as log_zip:
                self.assertIsNone(log_zip.testzip(),
                                  "Logfiles zip archive is broken")
    ...
    def test_wildcard_tasks_1(self):
        self.run_benchexec_and_compare_expected_files(
            '--tasks',
            '*',
            tasks=['DTD files', 'Markdown files', 'XML files', 'Dummy tasks'])

    def test_wildcard_tasks_2(self):
        self.run_benchexec_and_compare_expected_files(
            '--tasks',
            '* files',
            tasks=['DTD files', 'Markdown files', 'XML files'])
    ...
    def test_wildcard_tasks_6(self):
        self.run_benchexec_and_compare_expected_files(
            '--tasks', '[!D]*', tasks=['Markdown files', 'XML files'])
    ...
    def test_wildcard_rundefinition_1(self):
        self.run_benchexec_and_compare_expected_files(
            ...
            test_file=os.path.join(base_dir, 'doc',
                                   'benchmark-example-true.xml'),
    ...
    def test_validate_result_xml(self):
        ...
        xml_files = map(lambda x: os.path.join(self.tmp, basename + x),
                        xml_files)
