...
class TestCpuCoresPerRun(unittest.TestCase):
    ...
    def assertInvalid(self, coreLimit, num_of_threads):
        self.assertRaises(SystemExit, _get_cpu_cores_per_run0, coreLimit,
                          num_of_threads, *self.machine())
        ...
        if self.ht:
            ...
            singleThread_assignment = list(
                itertools.chain(
                    *zip(
                        range(core_count // 2),
                        range(core_count // 2, core_count))))
        else:
            singleThread_assignment = lrange(0, core_count)
        for coreLimit in range(1, core_count + 1):
            self.assertValid(coreLimit, 1,
                             [sorted(singleThread_assignment[:coreLimit])])
    ...
    def test_oneCorePerRun(self):
        ...
        for num_of_threads in range(1, maxThreads + 1):
            self.assertValid(1, num_of_threads,
                             self.oneCore_assignment[:num_of_threads])
    ...
    def test_twoCoresPerRun(self):
        ...
        if not self.twoCore_assignment:
            self.skipTest("Need result specified")
        for num_of_threads in range(1, maxThreads + 1):
            self.assertValid(2, num_of_threads,
                             self.twoCore_assignment[:num_of_threads])
    ...
    def test_threeCoresPerRun(self):
        ...
        if not self.threeCore_assignment:
            self.skipTest("Need result specified")
        for num_of_threads in range(1, maxThreads + 1):
            self.assertValid(3, num_of_threads,
                             self.threeCore_assignment[:num_of_threads])
    ...
    def test_fourCoresPerRun(self):
        ...
        if not self.fourCore_assignment:
            self.skipTest("Need result specified")
        for num_of_threads in range(1, maxThreads + 1):
            self.assertValid(4, num_of_threads,
                             self.fourCore_assignment[:num_of_threads])
    ...
    def test_eightCoresPerRun(self):
        ...
        if not self.eightCore_assignment:
            self.skipTest("Need result specified")
        for num_of_threads in range(1, maxThreads + 1):
            self.assertValid(8, num_of_threads,
                             self.eightCore_assignment[:num_of_threads])
...
class TestCpuCoresPerRun_singleCPU_HT(TestCpuCoresPerRun_singleCPU):
    ...
    def test_halfPhysicalCore(self):
        ...
        self.assertRaises(SystemExit, _get_cpu_cores_per_run0, 1, 1, [0],
                          {0: [0, 1]}, {0: [0, 1]})
...
class TestCpuCoresPerRun_dualCPU_HT(TestCpuCoresPerRun):
    ...
    twoCore_assignment = [[0, 16], [8, 24], [1, 17], [9, 25], [2, 18], [10, 26],
                          [3, 19], [11, 27], [4, 20], [12, 28], [5, 21],
    ...
    threeCore_assignment = [[0, 1, 16], [8, 9, 24], [2, 3, 18], [10, 11, 26],
                            [4, 5, 20], [12, 13, 28], [6, 7, 22], [14, 15, 30],
                            [17, 19, 21], [25, 27, 29]]

    fourCore_assignment = [[0, 1, 16, 17], [8, 9, 24, 25], [2, 3, 18, 19],
                           [10, 11, 26, 27], [4, 5, 20, 21], [12, 13, 28, 29],
                           [6, 7, 22, 23], [14, 15, 30, 31]]

    eightCore_assignment = [[0, 1, 2, 3, 16, 17, 18,
                             19], [8, 9, 10, 11, 24, 25, 26,
    ...
    def test_dualCPU_HT(self):
        self.assertValid(
            16, 2,
            [lrange(0, 8) + lrange(16, 24),
             lrange(8, 16) + lrange(24, 32)])
...
class TestCpuCoresPerRun_threeCPU_HT(TestCpuCoresPerRun):
    ...
    oneCore_assignment = [[x] for x in [
        0, 5, 10, 1, 6, 11, 2, 7, 12, 3, 8, 13, 4, 9, 14, 15, 20, 25, 16, 21,
        26, 17, 22, 27, 18, 23, 28, 19, 24, 29
    ]]
    twoCore_assignment = [[0, 15], [5, 20], [10, 25], [1, 16], [6, 21],
                          [11, 26], [2, 17], [7, 22], [12, 27], [3, 18],
                          [8, 23], [13, 28], [4, 19], [9, 24], [14, 29]]
    threeCore_assignment = [[0, 1, 15], [5, 6, 20], [10, 11, 25], [2, 3, 17],
                            [7, 8, 22], [12, 13, 27], [4, 16, 19], [9, 21, 24],
                            [14, 26, 29]]
    fourCore_assignment = [[0, 1, 15, 16], [5, 6, 20, 21], [10, 11, 25, 26],
                           [2, 3, 17, 18], [7, 8, 22, 23], [12, 13, 27, 28]]
    eightCore_assignment = [[0, 1, 2, 3, 15, 16, 17,
                             18], [5, 6, 7, 8, 20, 21, 22, 23],
    ...
    def test_threeCPU_HT_noncontiguousId(self):
        ...
        result = _get_cpu_cores_per_run0(2, 3, [0, 1, 2, 3, 6, 7], {
            0: [0, 1],
            2: [2, 3],
            3: [6, 7]
        }, {
        ...
        self.assertEqual([[0, 1], [2, 3], [6, 7]], result,
                         "Incorrect result for {} cores and {} threads.".format(
                             2, 3))
...
class TestCpuCoresPerRun_quadCPU_HT(TestCpuCoresPerRun):
    ...
    def test_quadCPU_HT_noncontiguousId(self):
        ...
        self.assertEqual([[0], [32], [48], [16], [8], [40], [56], [24]], result,
                         "Incorrect result for {} cores and {} threads.".format(
                             1, 8))
