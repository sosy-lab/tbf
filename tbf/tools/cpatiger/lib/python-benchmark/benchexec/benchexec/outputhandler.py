...
class OutputHandler(object):
    ...
    def __init__(self, benchmark, sysinfo, compress_results):
        ...
        self.write_header_to_log(version, memlimit, timelimit, corelimit,
                                 sysinfo)
    ...
    def store_header_in_xml(self, version, memlimit, timelimit, corelimit):
        ...
        self.xml_header = ET.Element(
            "result", {
                ...
                "tool":
                self.benchmark.tool_name,
                "version":
                version,
                "toolmodule":
                self.benchmark.tool_module,
                "generator":
    ...
    def writeRunSetInfoToLog(self, runSet):
        ...
        runSetInfo += "Run set {0} of {1} with options '{2}' and propertyfile '{3}'\n\n".format(
            ...
            " ".join(runSet.options), runSet.propertyfile)

        titleLine = self.create_output_line(runSet, "inputfile", "status",
                                            "cpu time", "wall time", "host",
                                            self.benchmark.columns, True)
    ...
    def output_before_run(self, run):
        ...
        try:
            ...
            terminalTitle = TERMINAL_TITLE.format(
                runSet.full_name +
                progressIndicator) if USE_COLORS and sys.stdout.isatty() else ""
            if self.benchmark.num_of_threads == 1:
                util.printOut(
                    terminalTitle + timeStr + self.format_sourcefile_name(
    ...
    def output_after_run(self, run):
        ...
        run.resultline = self.create_output_line(
            run.runSet, run.identifier, run.status, cputime_str, walltime_str,
            run.values.get('host'), run.columns)
        ...
        try:
            ...
            if self.benchmark.num_of_threads == 1:
                ...
                util.printOut(timeStr + self.format_sourcefile_name(
                    run.identifier, run.runSet) + valueStr)
        ...
        if self.compress_results:
            self.log_zip.write(
                ...
                os.path.relpath(
                    run.log_file,
                    os.path.join(self.benchmark.log_folder, os.pardir)))
    ...
    def run_set_to_text(self,
        ...
        if finished:
            ...
            cputime_str = "None" if cputime is None else util.format_number(
                cputime, TIME_PRECISION)
            walltime_str = "None" if walltime is None else util.format_number(
                walltime, TIME_PRECISION)
            lines.append(
                self.create_output_line(runSet, endline, "done", cputime_str,
                                        walltime_str, "-", []))
    ...
    def runs_to_xml(self, runSet, runs, blockname=None):
        ...
        if blockname is not None:
            ...
            runsElem.set("name", ((runSet.real_name + ".")
                                  if runSet.real_name else "") + blockname)
    ...
    def add_column_to_xml(self, xml, title, value, prefix="", value_suffix=""):
        ...
        if hasattr(value,
                   '__getitem__') and not isinstance(value, (str, bytes)):
        ...
        if self.xml_file_names:
            ...
            def _find_file_relative(name):
                ...
                if not os.path.isfile(path):
                    path = os.path.join(
                        os.path.dirname(__file__), os.path.pardir, name)
                    if not os.path.isfile(path):
                        return None
    ...
    def _write_pretty_result_xml_to_file(self, xml, filename):
        ...
        with io.TextIOWrapper(
                open_func(actual_filename, 'wb'), encoding='utf-8') as file:
...
class Statistics(object):
    ...
    def add_result(self, run):
        ...
        self.dic[(run.category,
                  result.get_result_classification(run.status))] += 1
        self.score += result.score_for_task(run.identifier, run.properties,
                                            run.category, run.status)
        #if run.properties:
        self.max_score += result.score_for_task(run.identifier, run.properties,
