...
def main(argv=None):
    ...
    resource_args = parser.add_argument_group(
        "optional arguments for resource limits")
    ...
    resource_args.add_argument(
        ...
        metavar="SECONDS",
        help=
        '"soft" CPU time limit in seconds (command will be send the TERM signal at this time)'
    )
    resource_args.add_argument(
        "--walltimelimit",
        type=util.parse_timespan_value,
        metavar="SECONDS",
        help=
        'wall time limit in seconds (default is CPU time limit plus a few seconds)'
    )
    resource_args.add_argument(
        "--cores",
    ...
    environment_args.add_argument(
        ...
        help=
        "additional cgroup values that should be set for runs (e.g., 'cpu.shares=1000')"
    )
    environment_args.add_argument(
        "--dir",
        metavar="DIR",
        help=
        "working directory for executing the command (default is current directory)"
    )
    environment_args.add_argument(
        "--user",
        metavar="USER",
        help=
        "execute tool under given user account (needs password-less sudo setup, "
    ...
    if len(options.args) == 1 and options.args[0].startswith("{"):
        ...
        if "maxLogfileSize" in data:
            try:
                options.maxOutputSize = int(
                    data["maxLogfileSize"]
                ) * _BYTE_FACTOR * _BYTE_FACTOR  # MB to bytes
            except ValueError:
                options.maxOutputSize = util.parse_memory_value(
                    data["maxLogfileSize"])
...
class RunExecutor(containerexecutor.ContainerExecutor):
    ...
    def __init__(self,
        ...
        if user is not None:
            ...
            logging.debug(
                'Checking for capability to run with sudo as user %s.', user)
            p = subprocess.Popen(
            ...
            try:
                if p.wait():
                    logging.error(
                        ...
                        ' '.join(sudo_check), p.returncode,
                        p.stdout.read().decode().strip(),
                        p.stderr.read().decode().strip())
                    sys.exit(
                        'Cannot execute benchmark as user "{0}", please fix your sudo setup.'.
                        format(user))
            finally:
                p.stdout.close()
                p.stderr.close()
    ...
    def _init_cgroups(self):
        ...
        for subsystem in self._cgroup_subsystems:
            ...
            if subsystem not in self.cgroups:
                sys.exit('Required cgroup subsystem "{}" is missing.'.format(
                    subsystem))
        ...
        if MEMORY not in self.cgroups:
            logging.warning(
                'Cannot measure memory consumption without memory cgroup.')
        else:
            if systeminfo.has_swap() and (not self.cgroups.has_value(
        ...
        if CPUSET in self.cgroups:
            ...
            try:
                self.cpus = util.parse_int_list(
                    self.cgroups.get_value(CPUSET, 'cpus'))
            except ValueError as e:
                logging.warning(
                    "Could not read available CPU cores from kernel: %s",
                    e.strerror)
            ...
            try:
                ...
                logging.warning(
                    "Could not read available memory nodes from kernel: %s",
                    e.strerror)
            logging.debug("List of available memory nodes is %s.",
                          self.memory_nodes)
    ...
    def _setup_cgroups(self, my_cpus, memlimit, memory_nodes, cgroup_values):
        ...
        for ((subsystem, option), value) in cgroup_values.items():
            try:
                ...
                cgroups.remove()
                sys.exit(
                    '{} for setting cgroup option {}.{} to "{}" (error code {}).'
    ...
    def _cleanup_temp_dir(self, base_dir):
        ...
        if self._should_cleanup_temp_dir:
            ...
            logging.info("Skipping cleanup of temporary directory %s.",
                         base_dir)
    ...
    def _setup_environment(self, environments):
        ...
        if self._user is not None or environments.get("keepEnv",
                                                      None) is not None:
            run_environment = {}
        else:
    ...
    def _setup_file_hierarchy_limit(self, files_count_limit, files_size_limit,
                                    temp_dir, cgroups, pid_to_kill):
    ...
    def execute_run(self,
        ...
        if softtimelimit is not None:
            ...
            if hardtimelimit and (softtimelimit > hardtimelimit):
                sys.exit(
                    "Soft time limit cannot be larger than the hard time limit."
                )
        ...
        if cores is not None:
            ...
            if not set(cores).issubset(self.cpus):
                sys.exit("Cores {0} are not allowed to be used".format(
        ...
        if memory_nodes is not None:
            ...
            if not set(memory_nodes).issubset(self.memory_nodes):
                sys.exit("Memory nodes {0} are not allowed to be used".format(
                    list(set(memory_nodes).difference(self.memory_nodes))))

        if workingDir:
            if not os.path.exists(workingDir):
                sys.exit(
                    "Working directory {0} does not exist.".format(workingDir))
            if not os.path.isdir(workingDir):
                sys.exit("Working directory {0} is not a directory.".format(
                    workingDir))
            if not os.access(workingDir, os.X_OK):
                sys.exit("Permission denied for working directory {0}.".format(
                    workingDir))
        ...
        for ((subsystem, option), _) in cgroupValues.items():
            if not subsystem in self._cgroup_subsystems:
                sys.exit(
                    ...
                    'Please specify "--require-cgroup-subsystem {subsystem}".'.
                    format(option=option, subsystem=subsystem))
            if not self.cgroups.has_value(subsystem, option):
                sys.exit(
                    'Cannot set option "{option}" for subsystem "{subsystem}", it does not exist.'
                    .format(option=option, subsystem=subsystem))

        if files_count_limit is not None:
            if files_count_limit < 0:
                sys.exit(
                    "Invalid files-count limit {0}.".format(files_count_limit))
        if files_size_limit is not None:
            if files_size_limit < 0:
                sys.exit(
                    "Invalid files-size limit {0}.".format(files_size_limit))

        try:
            return self._execute(args, output_filename, stdin, hardtimelimit,
                                 softtimelimit, walltimelimit, memlimit, cores,
                                 memory_nodes, cgroupValues, environments,
                                 workingDir, maxLogfileSize, files_count_limit,
                                 files_size_limit, **kwargs)
            ...
            logging.critical("OSError %s while starting '%s' in '%s': %s.",
                             e.errno, util.escape_string_shell(args[0]),
                             workingDir or '.', e.strerror)
            return {
                'terminationreason': 'failed',
                'exitcode': 0,
                'cputime': 0,
    ...
    def _execute(self, args, output_filename, stdin, hardtimelimit,
                 softtimelimit, walltimelimit, memlimit, cores, memory_nodes,
                 cgroup_values, environments, workingDir, max_output_size,
                 files_count_limit, files_size_limit, **kwargs):
        """
        ...
        if self._energy_measurement is not None:
            ...
            if cores is None:
                ...
                all_siblings = set(
                    util.flatten(
                        resources.get_cores_of_same_package_as(core)
                        for core in cores))
        ...
        def postParent(preParent_result):
            ...
            ) if self._energy_measurement else None
        ...
        try:
            ...
            timelimitThread = self._setup_cgroup_time_limit(
                hardtimelimit, softtimelimit, walltimelimit, cgroups, cores,
                pid)
            ...
            returnvalue, ru_child, (
                walltime,
                energy) = result_fn()  # blocks until process has terminated
    ...
    def _get_cgroup_measurements(self, cgroups, ru_child, result):
        ...
        if CPUACCT in cgroups:
            ...
            if cputime_wait > 0.5 and (cputime_wait * 0.95) > cputime_cgroups:
                logging.warning(
                    ...
                    'perhaps measurement is flawed.', cputime_wait,
                    cputime_cgroups)
            ...
            for (core, coretime) in enumerate(
                ...
                try:
                    ...
                    if coretime != 0:
                        result['cputime-cpu' + str(
                            core
                        )] = coretime / 1000000000  # nano-seconds to seconds
                except (OSError, ValueError) as e:
                    logging.debug(
                        "Could not read CPU time for core %s from kernel: %s",
                        core, e)
        else:
        ...
        if MEMORY in cgroups:
            ...
            if not cgroups.has_value(MEMORY, memUsageFile):
                logging.warning(
                    'Memory-usage is not available due to missing files.')
            else:
                try:
                    result['memory'] = int(
                        cgroups.get_value(MEMORY, memUsageFile))
                except IOError as e:
                    if e.errno == errno.ENOTSUP:  # kernel responds with operation unsupported if this is disabled
                        logging.critical(
        ...
        if BLKIO in cgroups:
            ...
            if cgroups.has_value(BLKIO, blkio_bytes_file):
                bytes_read = 0
                bytes_written = 0
                for blkio_line in cgroups.get_file_lines(
                        BLKIO, blkio_bytes_file):
                    try:
        ...
        logging.debug(
            ...
            result.get('walltime'), cputime_wait, cputime_cgroups,
...
def _reduce_file_size_if_necessary(fileName, maxSize):
    ...
    if maxSize is None:
        logging.debug("Size of logfile '%s' is %s bytes, size limit disabled.",
                      fileName, fileSize)
        return  # disabled, nothing to do

    if fileSize < (maxSize + 500):
        logging.debug("Size of logfile '%s' is %s bytes, nothing to do.",
                      fileName, fileSize)
        return

    logging.warning("Logfile '%s' is too big (size %s bytes). Removing lines.",
                    fileName, fileSize)
...
def _get_debug_output_after_crash(output_filename):
    ...
    try:
                ...
                if foundDumpFile:
                    ...
                    try:
                        ...
                        logging.warning(
                            ...
                            'from %s (%s)', dumpFileName, e.strerror)
                    break
                try:
        ...
        logging.warning(
            'Could not analyze tool output for crash information (%s)',
            e.strerror)
    ...
    def run(self):
        while not self.finished.is_set():
            ...
            if remainingCpuTime <= 0:
                ...
                logging.debug('Killing process %s due to CPU time timeout.',
                              self.pid_to_kill)
            ...
            if remainingWallTime <= 0:
                ...
                logging.warning('Killing process %s due to wall time timeout.',
                                self.pid_to_kill)
                self.kill_process(self.pid_to_kill, self.cgroups)
                self.finished.set()
                return
