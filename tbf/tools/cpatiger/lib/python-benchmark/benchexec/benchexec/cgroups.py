...
def find_my_cgroups(cgroup_paths=None):
    ...
    for subsystem, mount in _find_cgroup_mounts():
        ...
        if os.access(mount, os.F_OK):
            cgroupsParents[subsystem] = os.path.join(mount,
...
def kill_all_tasks_in_cgroup(cgroup, kill_process_fn):
    ...
    while True:
        ...
        for sig in [signal.SIGKILL, signal.SIGINT, signal.SIGTERM]:
                ...
                for task in tasks:
                    ...
                    if i > 1:
                        logging.warning(
                            ...
                            'in cgroup %s, sending signal %s (try %s).', task,
                            cgroup, sig, i)
            ...
            time.sleep(
                i *
                0.5)  # wait for the process to exit, this might take some time


def remove_cgroup(cgroup):
    if not os.path.exists(cgroup):
        logging.warning('Cannot remove CGroup %s, because it does not exist.',
                        cgroup)
        return
    ...
    try:
        ...
        try:
            ...
            logging.warning("Failed to remove cgroup %s: error %s (%s)", cgroup,
                            e.errno, e.strerror)
...
class Cgroup(object):
    ...
    def create_fresh_child_cgroup(self, *subsystems):
        ...
        for subsystem in subsystems:
            ...
            cgroup = tempfile.mkdtemp(
                prefix=CGROUP_NAME_PREFIX, dir=parentCgroup)
            ...
            def copy_parent_to_child(name):
                shutil.copyfile(
                    os.path.join(parentCgroup, name), os.path.join(
                        cgroup, name))
    ...
    def get_all_tasks(self, subsystem):
        ...
        """
        with open(os.path.join(self.per_subsystem[subsystem], 'tasks'),
                  'r') as tasksFile:
    ...
    def has_value(self, subsystem, option):
        ...
        return os.path.isfile(
            os.path.join(self.per_subsystem[subsystem],
                         subsystem + '.' + option))
    ...
    def read_cputime(self):
        ...
        return float(self.get_value(
            CPUACCT, 'usage')) / 1000000000  # nano-seconds to seconds
