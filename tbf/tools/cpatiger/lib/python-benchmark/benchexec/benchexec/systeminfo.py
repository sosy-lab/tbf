...
class SystemInfo(object):
    ...
    def __init__(self):
        ...
        if os.path.isfile(cpuInfoFilename) and os.access(
            ...
            cpuInfoLines = [
                tuple(line.split(':')) for line in cpuInfoFile.read()
                .replace('\n\n', '\n').replace('\t', '').strip('\n').split('\n')
            ]
        ...
        if 'cpu MHz' in cpuInfo:
            self.cpu_max_frequency = int(float(
        ...
        if os.path.isfile(freqInfoFilename) and os.access(
                freqInfoFilename, os.R_OK):
        ...
        if os.path.isfile(memInfoFilename) and os.access(
            ...
            memInfo = dict(
                tuple(s.split(': ')) for s in memInfoFile.read()
                .replace('\t', '').strip('\n').split('\n'))
            memInfoFile.close()
...
def is_turbo_boost_enabled():
    ...
    try:
        ...
        sys.exit(
            "Could not read turbo-boost information from kernel: {0}".format(e))
