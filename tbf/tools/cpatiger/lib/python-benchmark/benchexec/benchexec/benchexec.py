...
class BenchExec(object):
    ...
    def start(self, argv):
        ...
        if os.path.isdir(self.config.output_path):
            self.config.output_path = os.path.normpath(
                self.config.output_path) + os.sep
        ...
        if self.config.commit and not self.stopped_by_interrupt:
            try:
                util.add_files_to_git_repository(
                    self.config.output_path, output_handler.all_created_files,
    ...
    def check_existing_results(self, benchmark):
        ...
        if os.path.exists(benchmark.log_folder):
            sys.exit(
                'Output directory {0} already exists, will not overwrite existing results.'.
                format(benchmark.log_folder))
        if os.path.exists(benchmark.log_zip):
            sys.exit(
                'Output archive {0} already exists, will not overwrite existing results.'.
                format(benchmark.log_zip))
