...
def add_basic_executor_options(argument_parser):
    ...
    argument_parser.add_argument(
        ...
        metavar="ARG",
        help=
        'command line to run (prefix with "--" to ensure all arguments are treated correctly)'
    )
    argument_parser.add_argument(
...
class BaseExecutor(object):
    ...
    def __init__(self):
        ...
        )  # needed, because we kill the process asynchronous
    ...
    def _start_execution(self, args, stdin, stdout, stderr, env, cwd, temp_dir,
                         cgroups, parent_setup_fn, child_setup_fn,
                         parent_cleanup_fn):
        ...
        env["TEMPDIR"] = tmp_dir
        env["TEMP"] = tmp_dir
        logging.debug("Executing run with $HOME and $TMPDIR below %s.",
                      temp_dir)
