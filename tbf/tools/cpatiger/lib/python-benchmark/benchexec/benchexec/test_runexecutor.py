...
class TestRunExecutor(unittest.TestCase):
    ...
    def execute_run(self, *args, **kwargs):
        (output_fd, output_filename) = tempfile.mkstemp(
            '.log', 'output_', text=True)
        try:
            result = self.runexecutor.execute_run(
                list(args), output_filename, **kwargs)
    ...
    def execute_run_extern(self, *args, **kwargs):
        ...
        result = {
            key.strip(): value.strip() for (key, _, value) in (
    ...
    def check_result_keys(self, result, *additional_keys):
        ...
        for key in result.keys():
            if key.startswith('cputime-cpu'):
                ...
                self.assertRegex(
                    key, '^cpuenergy-pkg[0-9]+(-(core|uncore|dram))?$',
                    "unexpected result entry '{}={}'".format(key, result[key]))
            else:
                self.assertIn(
                    key, expected_keys,
    ...
    def test_command_output(self):
        ...
        self.assertEqual(output[-1], 'TEST_TOKEN',
                         'run output misses command output')
    ...
    def test_cputime_hardlimit(self):
        ...
        if 'terminationreason' in result:
            ...
            self.assertEqual(result['terminationreason'], 'cputime',
        ...
        self.assertAlmostEqual(
            ...
            delta=0.5,
            msg=
            'walltime is not approximately the time after which the process should have been killed'
        )
        self.assertAlmostEqual(
            result['cputime'],
            1.4,
            delta=0.5,
            msg=
            'cputime is not approximately the time after which the process should have been killed'
        )
    ...
    def test_cputime_softlimit(self):
        ...
        self.assertEqual(result['terminationreason'], 'cputime-soft',
    ...
    def test_walltime_limit(self):
        ...
        self.check_exitcode(result, 9, 'exit code of killed process is not 9')
        self.assertEqual(result['terminationreason'], 'walltime',
    ...
    def test_input_is_redirected_from_stdin(self):
        ...
        (output_fd, output_filename) = tempfile.mkstemp(
            '.log', 'output_', text=True)
        cmd = [
            runexec, '--input', '-', '--output', output_filename, '--walltime',
            '1', '/bin/cat'
        ]
        try:
            ...
            if retcode:
                ...
                raise subprocess.CalledProcessError(
                    retcode, cmd, output=runexec_output)
        ...
        result = {
            ...
            for (key, _,
                 value) in (line.partition('=')
                            for line in runexec_output.decode().splitlines())
        }
        self.check_exitcode(result, 0, 'exit code of process is not 0')
        self.assertAlmostEqual(
    ...
    def test_stop_run(self):
        ...
        self.check_exitcode(result, 9, 'exit code of killed process is not 9')
        self.assertEqual(result['terminationreason'], 'killed',
                         'termination reason is not "killed"')
        self.assertAlmostEqual(
            result['walltime'],
            1,
            delta=0.5,
    ...
    def test_reduce_file_size_limit_zero(self):
        with tempfile.NamedTemporaryFile(mode='wt') as tmp:
            ...
            runexecutor._reduce_file_size_if_necessary(tmp.name, 0)
            self.assertLessEqual(
                os.path.getsize(tmp.name), self.REDUCE_OVERHEAD)
            with open(tmp.name, 'rt') as tmp2:
                new_content = tmp2.read()
        self.assertIn(self.REDUCE_WARNING_MSG, new_content)
        self.assertTrue(new_content.startswith(line))
    ...
    def test_home_and_tmp_is_separate(self):
        ...
        self.assertRegex(
            output[-1], '/BenchExec_run_[^/]*/home .*/BenchExec_run_[^/]*/tmp',
    ...
    def test_no_cleanup_temp(self):
        if not os.path.exists('/bin/sh'):
            self.skipTest('missing /bin/sh')
        self.setUp(
            cleanup_temp_dir=False)  # create RunExecutor with desired parameter
        (result, output) = self.execute_run(
        ...
        subprocess.check_call(
            self.runexecutor._build_cmdline(['test', '-f', test_file]))
        self.assertEqual('tmp', os.path.basename(temp_dir),
                         'unexpected name of temp dir')
        self.assertNotEqual('/tmp', temp_dir,
                            'temp dir should not be the global temp dir')
        subprocess.check_call(
            self.runexecutor._build_cmdline(
    ...
    def test_require_cgroup_cpu(self):
        ...
        for line in output:
            if re.match('^[0-9]*:([^:]*,)?cpu(,[^:]*)?:/(.*/)?benchmark_.*$',
                        line):
                return  # Success
...
class TestRunExecutorWithContainer(TestRunExecutor):
    ...
    def setUp(self, *args, **kwargs):
        try:
            ...
            self.skipTest("Namespaces not supported: {}".format(
                os.strerror(e.errno)))
    ...
    def test_home_is_writable(self):
        self.skipTest(
            "needs container_system_config=True and thus overlay mode")

    def test_no_cleanup_temp(self):
        self.skipTest("not relevant in container")

    def check_result_files(self, shell_cmd, result_files_patterns,
    ...
    def test_result_file_wildcard(self):
        self.check_result_files(
            "echo TEST_TOKEN > TEST_FILE; "
            "echo TEST_TOKEN > TEST_FILE2; "
            "echo TEST_TOKEN > TEST_NOFILE; ", ["TEST_FILE*"],
            ["TEST_FILE", "TEST_FILE2"])
    ...
    def test_result_file_recursive_pattern(self):
        ...
        self.check_result_files(
            ...
            "echo TEST_TOKEN > TEST_DIR/TEST_DIR/TEST_FILE.txt; ", ["**/*.txt"],
            [
                "TEST_FILE.txt", "TEST_DIR/TEST_FILE.txt",
    ...
    def test_file_count_limit(self):
        ...
        self.assertEqual(result['terminationreason'], 'files-count',
    ...
    def test_file_size_limit(self):
        ...
        self.check_exitcode(result, 9, 'exit code of killed process is not 15')
        self.assertEqual(result['terminationreason'], 'files-size',
