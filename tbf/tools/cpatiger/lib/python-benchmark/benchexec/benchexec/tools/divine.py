...
class Tool(benchexec.tools.template.BaseTool):
    ...
    LIBS = [
        'libc++abi.so.1', 'libc.so.6', 'libdl.so.2', 'libm.so.6', 'librt.so.1',
        'libunwind.so.1', 'libc++.so.1', 'libdivinert.bc', 'libgcc_s.so.1',
        'libpthread.so.0', 'libtinfo.so.5', 'libz.so.1'
    ]
    ...
    def cmdline(self, executable, options, tasks, propertyfile=None,
        ...
        run = [os.path.join('.', directory, self.BINS[1]), directory
              ] + options + tasks
        return run
    ...
    def program_files(self, executable):
        ...
        directory = os.path.dirname(executable)
        return (
            [os.path.join('.', directory, x) for x in self.BINS] +
