...
class Tool(benchexec.tools.template.BaseTool):
    ...
    def executable(self):
        ...
        executableDir = os.path.join(
            os.path.dirname(executable), os.path.pardir)
    ...
    def version(self, executable):
        ...
        line = next(
            l for l in stdout.splitlines() if l.startswith('CPAchecker'))
    ...
    def determine_result(self, returncode, returnsignal, output, isTimeout):
        ...
        def isOutOfNativeMemory(line):
            return ('std::bad_alloc' in
                    line  # C++ out of memory exception (MathSAT)
                    or 'Cannot allocate memory' in line or
                    'Native memory allocation (malloc) failed to allocate' in
                    line  # JNI
        ...
        for line in output:
            if 'java.lang.OutOfMemoryError' in line:
                ...
                status = 'SEGMENTATION FAULT'
                  ...
                  returncode == 1) and 'java.lang.AssertionError' in line:
                status = 'ASSERTION'
            elif (
                (returncode == 0 or returncode == 1) and
                ...
                if line.startswith('TRUE'):
                    ...
                    if match and match.group(1) in [
                            'valid-deref', 'valid-free', 'valid-memtrack',
                            'no-overflow', 'no-deadlock', 'termination'
                    ]:
        ...
        if (not status or status == result.RESULT_UNKNOWN
           ) and isTimeout and returncode in [15, 143]:
        ...
        for line in lines:
            if identifier in line:
                ...
                endPosition = line.find(
                    '(', startPosition)  # bracket maybe not found -> (-1)
                if (endPosition == -1):
