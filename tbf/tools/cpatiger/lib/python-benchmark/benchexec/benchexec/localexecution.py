...
def init(config, benchmark):
    ...
    try:
        processes = subprocess.Popen(
            ['ps', '-eo', 'cmd'], stdout=subprocess.PIPE).communicate()[0]
        if len(
                re.findall("python.*benchmark\.py",
...
def execute_benchmark(benchmark, output_handler):
    ...
    if CORELIMIT in benchmark.rlimits:
        ...
        coreAssignment = get_cpu_cores_per_run(
            benchmark.rlimits[CORELIMIT], benchmark.num_of_threads, my_cgroups)
    ...
    if benchmark.num_of_threads > 1 and benchmark.config.users:
        if len(benchmark.config.users) == 1:
            ...
            benchmark.config.users = [
                benchmark.config.users[0]
                for i in range(benchmark.num_of_threads)
            ]
        elif len(benchmark.config.users) < benchmark.num_of_threads:
            sys.exit(
                'Distributing parallel runs to different user accounts was requested, but not enough accounts were given. Please specify {} user accounts, or only one account.'.
                format(benchmark.num_of_threads))
        elif len(benchmark.config.users) != len(set(benchmark.config.users)):
            sys.exit(
                'Same user account was specified multiple times, please specify {} separate accounts, or only one account.'.
                format(benchmark.num_of_threads))
    ...
    for runSet in benchmark.run_sets:
        ...
        if not runSet.should_be_executed():
            ...
            for i in range(benchmark.num_of_threads):
                ...
                user = benchmark.config.users[
                    i] if benchmark.config.users else None
            ...
            if energy and cpu_packages:
                energy = {
                    pkg: energy[pkg] for pkg in energy if pkg in cpu_packages
                }
...
class _Worker(threading.Thread):
    ...
    def __init__(self, benchmark, my_cpus, my_memory_nodes, my_user,
        ...
        self.run_executor = RunExecutor(
            user=my_user, **benchmark.config.containerargs)
        self.setDaemon(True)
