...
def get_cpu_cores_per_run(coreLimit, num_of_threads, my_cgroups):
    ...
    try:
        ...
        for core in allCpus:
            siblings = util.parse_int_list(
                util.read_file(
                    '/sys/devices/system/cpu/cpu{0}/topology/thread_siblings_list'.
                    format(core)))
    ...
    return _get_cpu_cores_per_run0(coreLimit, num_of_threads, allCpus,
                                   cores_of_package, siblings_of_core)


def _get_cpu_cores_per_run0(coreLimit, num_of_threads, allCpus,
                            cores_of_package, siblings_of_core):
    ...
    if coreLimit > len(allCpus):
        sys.exit(
            "Cannot run benchmarks with {0} CPU cores, only {1} CPU cores available.".
            format(coreLimit, len(allCpus)))
    if coreLimit * num_of_threads > len(allCpus):
        sys.exit(
            "Cannot run {0} benchmarks in parallel with {1} CPU cores each, only {2} CPU cores available. Please reduce the number of threads to {3}.".
            format(num_of_threads, coreLimit, len(allCpus),
                   len(allCpus) // coreLimit))
    ...
    for package, cores in cores_of_package.items():
        if package_size is None:
            ...
            sys.exit(
                "Asymmetric machine architecture not supported: CPU package {0} has {1} cores, but other package has {2} cores.".
                format(package, len(cores), package_size))
    ...
    for core, siblings in siblings_of_core.items():
        if core_size is None:
            ...
            sys.exit(
                "Asymmetric machine architecture not supported: CPU core {0} has {1} siblings, but other core has {2} siblings.".
                format(core, len(siblings), core_size))
    ...
    for core, siblings in siblings_of_core.items():
        ...
        if not siblings_set.issubset(all_cpus_set):
            sys.exit(
                "Core assignment is unsupported because siblings {0} of core {1} are not usable. Please always make all virtual cores of a physical core available.".
                format(siblings_set.difference(all_cpus_set), core))
    ...
    if packages_per_run > 1 and packages_per_run * num_of_threads > package_count:
        sys.exit(
            "Cannot split runs over multiple CPUs and at the same time assign multiple runs to the same CPU. Please reduce the number of threads to {0}.".
            format(package_count // packages_per_run))
    ...
    if packages_per_run == 1 and runs_per_package * coreLimit > package_size:
        sys.exit(
            "Cannot run {} benchmarks with {} cores on {} CPUs with {} cores, because runs would need to be split across multiple CPUs. Please reduce the number of threads.".
            format(num_of_threads, coreLimit, package_count, package_size))
    ...
    if packages_per_run == 1:
        ...
        if coreLimit_rounded_up * runs_per_package > package_size:
            ...
            logging.warning(
                ...
                (package_size // coreLimit_rounded_up) * package_count)
    ...
    logging.debug(
        "Going to assign at most %s runs per package, each one using %s cores and blocking %s cores on %s packages.",
        runs_per_package, coreLimit, coreLimit_rounded_up, packages_per_run)
    ...
    for run in range(num_of_threads):
        ...
        for package_nr in range(start_package,
                                start_package + packages_per_run):
            ...
            for core in cores_of_package[package]:
                if core not in cores:
                    cores.extend(c for c in siblings_of_core[core]
                                 if not c in used_cores)
                if len(cores) >= coreLimit:
                    break
            cores_with_siblings.update(cores)
            cores = cores[:
                          coreLimit]  # shrink if we got more cores than necessary
            # remove used cores such that we do not try to use them again
            cores_of_package[package] = [
                core for core in cores_of_package[package] if core not in cores
            ]

        assert len(
            cores
        ) == coreLimit, "Wrong number of cores for run {} of {} - previous results: {}, remaining cores per package: {}, current cores: {}".format(
            run + 1, num_of_threads, result, cores_of_package, cores)
    ...
    assert len(
        set(itertools.chain(*result))
    ) == num_of_threads * coreLimit, "Cores are not uniquely assigned to runs: " + result
...
def _get_memory_banks_listed_in_dir(path):
    ...
    return [
        int(entry[4:]) for entry in os.listdir(path) if entry.startswith('node')
    ]
...
def check_memory_size(memLimit, num_of_threads, memoryAssignment, my_cgroups):
    ...
    try:
        ...
        def check_limit(actualLimit):
            if actualLimit < memLimit:
                sys.exit(
                    "Cgroups allow only {} bytes of memory to be used, cannot execute runs with {} bytes of memory.".
                    format(actualLimit, memLimit))
            elif actualLimit < memLimit * num_of_threads:
                sys.exit(
                    "Cgroups allow only {} bytes of memory to be used, not enough for {} benchmarks with {} bytes each. Please reduce the number of threads".
                    format(actualLimit, num_of_threads, memLimit))
        ...
        if cgroups.MEMORY in my_cgroups:
            ...
            for key, value in my_cgroups.get_key_value_pairs(
                    cgroups.MEMORY, 'stat'):
        ...
        if not memoryAssignment:
            if cgroups.CPUSET in my_cgroups:
                ...
                allMems = _get_memory_banks_listed_in_dir(
    ...
    for mems_of_run in memoryAssignment:
        ...
        if totalSize < memLimit:
            sys.exit(
                "Memory banks {} do not have enough memory for one run, only {} bytes available.".
                format(mems_of_run, totalSize))
        usedMem[tuple(mems_of_run)] += memLimit
        if usedMem[tuple(mems_of_run)] > totalSize:
            sys.exit(
                "Memory banks {} do not have enough memory for all runs, only {} bytes available. Please reduce the number of threads.".
                format(mems_of_run, totalSize))
    ...
    with open(fileName) as f:
        for line in f:
            if 'MemTotal' in line:
                ...
                if size[-3:] != ' kB':
                    raise ValueError(
                        '"{}" in file {} is not a memory size.'.format(
                            size, fileName))
                size = int(
                    size[:-3]
                ) * 1024  # kernel uses KiB but names them kB, convert to Byte
                logging.debug("Memory bank %s has size %s bytes.", memBank,
                              size)
                return size
...
def get_cpu_package_for_core(core):
    ...
    return int(
        util.read_file(
            '/sys/devices/system/cpu/cpu{0}/topology/physical_package_id'.
            format(core)))
...
def get_cores_of_same_package_as(core):
    return util.parse_int_list(
        util.read_file(
            '/sys/devices/system/cpu/cpu{0}/topology/core_siblings_list'.format(
                core)))
