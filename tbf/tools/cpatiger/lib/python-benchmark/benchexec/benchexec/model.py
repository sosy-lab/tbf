...
def substitute_vars(oldList, runSet=None, sourcefile=None):
    ...
    if runSet:
        ...
        keyValueList = [('${benchmark_name}',
                         benchmark.name), ('${benchmark_date}',
                                           benchmark.instance),
                        ('${benchmark_path}', benchmark.base_dir or
                         '.'), ('${benchmark_path_abs}',
                                os.path.abspath(benchmark.base_dir)),
                        ('${benchmark_file}',
                         os.path.basename(benchmark.benchmark_file)),
                        ('${benchmark_file_abs}',
                         os.path.abspath(
                             os.path.basename(benchmark.benchmark_file))),
                        ('${logfile_path}',
                         os.path.dirname(runSet.log_folder) or
                         '.'), ('${logfile_path_abs}',
                                os.path.abspath(runSet.log_folder)),
                        ('${rundefinition_name}', runSet.real_name
                         if runSet.real_name else
                         ''), ('${test_name}', runSet.real_name
                               if runSet.real_name else '')]
    ...
    if sourcefile:
        ...
        keyValueList.append(('${inputfile_path}', os.path.dirname(sourcefile) or
                             '.'))
        keyValueList.append(('${inputfile_path_abs}',
                             os.path.dirname(os.path.abspath(sourcefile))))
        # The following are deprecated: do not use anymore.
        keyValueList.append(('${sourcefile_name}',
                             os.path.basename(sourcefile)))
        keyValueList.append(('${sourcefile_path}',
                             os.path.dirname(sourcefile) or '.'))
        keyValueList.append(('${sourcefile_path_abs}',
                             os.path.dirname(os.path.abspath(sourcefile))))
...
def load_tool_info(tool_name):
    ...
    """
    tool_module = tool_name if '.' in tool_name else (
        "benchexec.tools." + tool_name)
    try:
        tool = __import__(tool_module, fromlist=['Tool']).Tool()
    except ImportError as ie:
        sys.exit('Unsupported tool "{0}" specified. ImportError: {1}'.format(
            tool_name, ie))
...
def cmdline_for_run(tool, executable, options, sourcefiles, propertyfile,
                    rlimits):
    ...
    args = tool.cmdline(rel_executable, list(options),
                        list(map(relpath, sourcefiles)),
                        relpath(propertyfile) if propertyfile else None,
                        rlimits.copy())
    assert all(
        args), "Tool cmdline contains empty or None argument: " + str(args)
...
class Benchmark(object):
    ...
    def __init__(self, benchmark_file, config, start_time):
        ...
        # get benchmark-name
        self.name = os.path.basename(benchmark_file)[:
                                                     -4]  # remove ending ".xml"
        if config.name:
        ...
        self.num_of_threads = int(
            rootTag.get("threads")) if ("threads" in keys) else 1
        ...
        # get benchmarks
        self.run_sets = []
        for (i, rundefinitionTag) in enumerate(
                rootTag.findall("rundefinition")):
        ...
        if not self.run_sets:
            for (i, rundefinitionTag) in enumerate(rootTag.findall("test")):
                self.run_sets.append(
                    RunSet(rundefinitionTag, self, i + 1,
                           globalSourcefilesTags))
            if self.run_sets:
                logging.warning(
                    "Benchmark file %s uses deprecated <test> tags. "
                    "Please rename them to <rundefinition>.", benchmark_file)
            else:
                logging.warning(
                    "Benchmark file %s specifies no runs to execute "
                    "(no <rundefinition> tags found).", benchmark_file)
            ...
            for selected in config.selected_run_definitions:
                if not any(
                    ...
                    logging.warning(
                        ...
                        'skipping it.', selected)

    def required_files(self):
        assert self.executable is not None, "executor needs to set tool executable"
        return self._required_files.union(
            self.tool.program_files(self.executable))
    ...
    def load_columns(columnsTag):
        ...
        if columnsTag != None:  # columnsTag is optional in XML file
            for columnTag in columnsTag.findall("column"):
                ...
                number_of_digits = columnTag.get(
                    "numberOfDigits")  # digits behind comma
...
class RunSet(object):
    ...
    def __init__(self,
        ...
        if self.real_name:
            ...
            self.result_files_folder = os.path.join(self.result_files_folder,
                                                    self.real_name)

        # get all run-set-specific options from rundefinitionTag
        self.options = benchmark.options + util.get_list_from_xml(
            rundefinitionTag)
        self.propertyfile = util.text_or_none(
            util.get_single_child_from_xml(
                rundefinitionTag, PROPERTY_TAG)) or benchmark.propertyfile
        ...
        self.blocks = self.extract_runs_from_xml(
            ...
            rundefinitionTag.findall("sourcefiles"), required_files_pattern)
        ...
        self.full_name = self.benchmark.name + (("." + self.name)
                                                if self.name else "")
    ...
    def extract_runs_from_xml(self, sourcefilesTagList,
        ...
        for index, sourcefilesTag in enumerate(sourcefilesTagList):
            ...
            sourcefiles = self.get_sourcefiles_from_xml(sourcefilesTag,
            ...
            for sourcefile in sourcefiles:
                currentRuns.append(
                    Run(
                        sourcefile, fileOptions, self, propertyfile,
    ...
    def get_sourcefiles_from_xml(self, sourcefilesTag, base_dir):
        ...
        for includedFiles in sourcefilesTag.findall("include"):
            sourcefiles += self.expand_filename_pattern(includedFiles.text,
                                                        base_dir)

        # get sourcefiles from list in file
        for includesFilesFile in sourcefilesTag.findall("includesfile"):

            for file in self.expand_filename_pattern(includesFilesFile.text,
                                                     base_dir):
                ...
                if util.is_code(file):
                    logging.error(
                        ...
                        "or remove bracket '{' from this file.", file)
                    sys.exit()
        ...
        for excludesFilesFile in sourcefilesTag.findall("excludesfile"):
            for file in self.expand_filename_pattern(excludesFilesFile.text,
                                                     base_dir):
                # read files from list
                fileWithList = open(file, 'rt')
                for line in fileWithList:
                    ...
                    if not util.is_comment(line):
                        excludedFilesList = self.expand_filename_pattern(
                            line, os.path.dirname(file))
                        for excludedFile in excludedFilesList:
                            sourcefiles = util.remove_all(
                                sourcefiles, excludedFile)
            ...
            for appendFile in appendFileTags:
                files.extend(
                    self.expand_filename_pattern(
                        appendFile.text, base_dir, sourcefile=sourcefile))
    ...
    def expand_filename_pattern(self, pattern, base_dir, sourcefile=None):
        ...
        if expandedPattern != pattern:
            logging.debug("Expanded variables in expression %r to %r.", pattern,
                          expandedPattern)
...
class Run(object):
    ...
    def cmdline(self):
        ...
        return cmdline_for_run(self.runSet.benchmark.tool,
                               self.runSet.benchmark.executable, self.options,
                               self.sourcefiles, self.propertyfile,
    ...
    def after_execution(self,
        ...
        self.status = self._analyze_result(exitcode, output, isTimeout,
                                           termination_reason)
        self.category = result.get_result_category(self.identifier, self.status,
                                                   self.properties)
    ...
    def _analyze_result(self, exitcode, output, isTimeout, termination_reason):
        ...
        if exitcode is not None:
            ...
            if tool_status in [result.RESULT_ERROR, result.RESULT_UNKNOWN]:
                ...
                if exitcode.signal == 6:
                    ...
                    tool_status = '{} ({})'.format(result.RESULT_ERROR,
                                                   exitcode.value)
        ...
        if not status:
            ...
            status = tool_status
                ...
                status, result.RESULT_ERROR, result.RESULT_UNKNOWN, 'KILLED',
                'KILLED BY SIGNAL 9'
        ]:
...
class Requirements(object):
    ...
    def __str__(self):
        ...
        if self.memory:
            s += " Memory=" + str(
                self.memory / _BYTE_FACTOR / _BYTE_FACTOR) + " MB"
