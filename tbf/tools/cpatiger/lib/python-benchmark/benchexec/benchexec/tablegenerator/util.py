...
def extend_file_list(filelist):
    ...
    '''
    return [
        file for wildcardFile in filelist
        for file in get_file_list(wildcardFile)
    ]
...
def format_options(options):
    ...
    return '<span style="display:block">' + '</span><span style="display:block">'.join(
        line for line in lines if line.strip()) + '</span>'
...
def merge_entries_with_common_prefixes(list_, number_of_needed_commons=6):
    ...
    lists_to_merge = []
    for entry in list_:
        newPrefix, number = split_string_at_suffix(
            entry, numbers_into_suffix=True)
    ...
    for common_entries in lists_to_merge:
        ...
        assert all(
            common_prefix == prefix for entry, prefix, number in common_entries)
        if len(common_entries) <= number_of_needed_commons:
            returnvalue.extend(
                (entry for entry, prefix, number in common_entries))
        else:
