...
TEMPLATE_FILE_NAME = os.path.join(
    os.path.dirname(__file__), 'template.{format}')
...
def parse_table_definition_file(file, options):
    ...
    if 'table' != tableGenFile.tag:
        logging.error(
            "Table file %s is invalid: It's root element is not named 'table'.",
            file)
        exit(1)

    defaultColumnsToShow = extract_columns_from_table_definition_file(
        tableGenFile, file)
    columns_relevant_for_diff = _get_columns_relevant_for_diff(
        defaultColumnsToShow)

    return Util.flatten(
        parallel.map(handle_tag_in_table_definition_file, tableGenFile,
...
def extract_columns_from_table_definition_file(xmltag, table_definition_file):
    ...
    for c in xmltag.findall('column'):
        ...
        new_column = Column(
            c.get("title"), c.text, c.get("numberOfDigits"),
...
def _get_decimal_digits(decimal_number_match, number_of_significant_digits):
    ...
    if int(decimal_number_match.group(GROUP_INT_PART)) == 0\
        ...
        curr_dec_digits = len(
            decimal_number_match.group(GROUP_ZEROES)) + int(num_of_digits)
        ...
        if value_match is None:
            ...
            if value_match.group(GROUP_DEC_PART):
                dec_digits_before_scale = len(
                    value_match.group(GROUP_DEC_PART)
                ) - 1  # - 1 since GROUP_DEC_PART includes the point
            else:
                dec_digits_before_scale = 0
            max_number_of_dec_digits_after_scale = max(
                0, dec_digits_before_scale - math.ceil(
...
def get_column_type(column, result_set):
    ...
    try:
        ...
        logging.error("Column type couldn't be determined: {}".format(
            e.message))
        return ColumnType.text, None, None, 1


def handle_tag_in_table_definition_file(tag, table_file, defaultColumnsToShow,
                                        columns_relevant_for_diff, options):

    def get_file_list(result_tag):
        if not 'filename' in result_tag.attrib:
            logging.warning(
                "Result tag without filename attribute in file '%s'.",
                table_file)
            return []
        return Util.get_file_list(
            os.path.join(
                os.path.dirname(table_file),
                result_tag.get('filename')))  # expand wildcards

    results = []  # default value for results
    if tag.tag == 'result':
        columnsToShow = extract_columns_from_table_definition_file(
            tag, table_file) or defaultColumnsToShow
    ...
    def load_tool_module(tool_module):
        ...
        try:
            ...
            logging.warning(
                ...
                'cannot extract values from log files.', tool_module)
        return None

    tool_module = result.attributes['toolmodule'][
        0] if 'toolmodule' in result.attributes else None
...
class RunSetResult(object):
    ...
    def __init__(self,
        ...
        self.columns = copy.deepcopy(
            columns)  # Copy the columns since they may be modified
    ...
    @staticmethod
    def create_from_xml(resultFile,
                        resultElem,
                        columns=None,
                        all_columns=False,
                        columns_relevant_for_diff=set()):
        ...
        return RunSetResult(
            _get_run_tags_from_xml(resultElem), attributes, columns, summary,
            columns_relevant_for_diff)

    @staticmethod
    def _extract_existing_columns_from_result(resultFile, resultElem,
                                              all_columns):
    ...
    def _extract_attributes_from_result(resultFile, resultTag):
        ...
        # Defaults
        attributes['branch'] = [
            os.path.basename(resultFile).split('#')[0]
            if '#' in resultFile else ''
        ]
...
def parse_results_file(resultFile, run_set_id=None, ignore_errors=False):
    ...
    if ignore_errors and 'error' in resultElem.attrib:
        logging.warning('Ignoring file "%s" because of error: %s', resultFile,
...
def insert_logfile_names(resultFile, resultElem):
    ...
    for sourcefile in _get_run_tags_from_xml(resultElem):
        if 'logfile' in sourcefile.attrib:
            log_file = urllib.parse.urljoin(resultFile,
                                            sourcefile.get('logfile'))
        else:
            log_file = log_folder + os.path.basename(
                sourcefile.get('name')) + ".log"
    ...
    for runset in runset_results:
        ...
        for task in runset.get_tasks():
            if task in currentresult_taskset:
                logging.warning("Task '%s' is present twice, skipping it.",
                                task[0])
            else:
...
def merge_task_lists(runset_results, tasks):
    ...
    for runset in runset_results:
        ...
        dic = dict([(run_result.task_id, run_result)
        ...
        for task in tasks:
            ...
            if run_result is None:
                logging.info(
                    "    no result for task '%s' (tool=%s, benchmark=%s, benchmark name=%s).",
                    task[0], runset.attributes['tool'],
                    runset.attributes['name'],
                    runset.attributes['benchmarkname'])
                # create an empty dummy element
                run_result = RunResult(task, None, result.CATEGORY_MISSING, 0,
                                       None, runset.columns,
...
class RunResult(object):
    ...
    def create_from_xml(sourcefileTag, get_value_from_logfile, listOfColumns,
        ...
        def read_logfile_lines(log_file):
            ...
            if log_zip_url.startswith(
                    "file:///") and not log_zip_path.startswith("/"):
                ...
                try:
                    ...
                    try:
                        with io.TextIOWrapper(
                                log_zip.open(path_in_zip)) as logfile:
                            return logfile.readlines()
                    except KeyError:
                        logging.warning(
            ...
            if column.title.lower() == 'score':
                ...
                if not column.pattern or column.href:
                    ...
                    if logfileLines is None:  # cache content
                        logfileLines = read_logfile_lines(
        ...
        return RunResult(
            get_task_id(sourcefileTag), status, category, score,
...
class Row(object):
    ...
    def __init__(self, results):
        ...
        assert len(set(
            r.task_id
            for r in results)) == 1, "not all results are for same task"
...
def filter_rows_with_differences(rows):
    ...
    def all_equal_result(listOfResults):
        ...
        for col in relevant_columns:
            ...
            status.append(
                set(res.values[get_index_of_column(col, res.columns)]
                    for res in listOfResults))
        ...
        for key in runSetResult.attributes:
            ...
            if key == 'turbo':
                ...
                if len(turbo_values) > 1:
                    ...
                    turbo = None
                runSetResult.attributes['turbo'] = ', Turbo Boost: {}'.format(
                    turbo) if turbo else ''
...
def select_relevant_id_columns(rows):
    ...
    if rows:
        ...
        for column in range(1, len(prototype_id)):
            ...
            relevant_id_columns.append(
                not all(map(id_equal_to_prototype, rows)))
...
def get_stats_of_rows(rows):
    ...
    for row in rows:
        if not row.properties:
            ...
            break
        correct_result = result.satisfies_file_property(row.filename,
                                                        row.properties)
        if correct_result is True:
            count_true += 1
        elif correct_result is False:
            count_false += 1
        max_score += result.score_for_task(row.filename, row.properties,
...
def _contains_unconfirmed_results(rows_for_stats):
    for unconfirmed_stat in rows_for_stats[
            4]:  # 5th position in rows_for_stats is 'unconfirmed_results'
...
def get_stats(rows, local_summary):
    stats = list(parallel.map(get_stats_of_run_set,
                              rows_to_columns(rows)))  # column-wise
    ...
    for i, column in enumerate(columns):
        ...
        column_type, column_unit, column_source_unit, column_scale_factor = _get_column_type_heur(
            column, column_values)
        new_column = Column(column.title, column.pattern,
                            column.number_of_significant_digits, column.href,
                            column_type, column_unit, column_source_unit,
                            column_scale_factor, column.display_title)
        stats_columns.append(new_column)

    max_score, count_true, count_false = get_stats_of_rows(rows)
    task_counts = 'in total {0} true tasks, {1} false tasks'.format(
        count_true, count_false)

    if max_score:
        score_row = tempita.bunch(
            id='score',
            title='score ({0} tasks, max score: {1})'.format(
                len(rows), max_score),
    ...
    if local_summary:
        summary_row = tempita.bunch(
            ...
            title='local summary',
            description=
            '(This line contains some statistics from local execution. Only trust those values, if you use your own computer.)',
    ...
    stats_info_correct_unconfirmed = [
        ...
        tempita.bunch(
            ...
            description=
            'property does not hold + result is false, but unconfirmed',
    ...
    return [
        tempita.bunch(
            ...
            title='total',
            description=task_counts,
            content=rowsForStats[0]),
    ] + ([summary_row] if local_summary else
         []) + stats_info + ([score_row] if max_score else []), stats_columns
        ...
        if col_type != ColumnType.text:
            if col_type == ColumnType.main_status or col_type == ColumnType.status:
                if col_type == ColumnType.main_status:
                    ...
                    score = StatValue(
                        sum(run_result.score for run_result in runResults))
                else:
                    score = None
                ...
                curr_status_list = [(runResult.category,
                                     runResult.values[index])
                                    for runResult in runResults]
                ...
                countCorrectTrue = counts[result.CATEGORY_CORRECT,
                                          result.RESULT_CLASS_TRUE]
                countCorrectFalse = counts[result.CATEGORY_CORRECT,
                                           result.RESULT_CLASS_FALSE]
                ...
                countCorrectUnconfirmedFalse = counts[
                    result.CATEGORY_CORRECT_UNCONFIRMED,
                    result.RESULT_CLASS_FALSE]
                countWrongTrue = counts[result.CATEGORY_WRONG,
                                        result.RESULT_CLASS_TRUE]
                countWrongFalse = counts[result.CATEGORY_WRONG,
                                         result.RESULT_CLASS_FALSE]
                ...
                correctUnconfirmed = StatValue(countCorrectUnconfirmedTrue +
                                               countCorrectUnconfirmedFalse)
                correctUnconfirmedTrue = StatValue(countCorrectUnconfirmedTrue)
                correctUnconfirmedFalse = StatValue(
                    countCorrectUnconfirmedFalse)
...
def get_stats_of_number_column(values, categoryList, columnTitle):
    ...
    try:
        ...
        if columnTitle != "host" and not columnTitle.endswith(
                'status'):  # We ignore values of columns 'host' and 'status'.
    ...
    for value, catStat in zip(valueList, categoryList):
        ...
        if status is None:
            continue
        valuesPerCategory[
            category, result.get_result_classification(status)].append(value)

    return (
        StatValue.from_list(valueList),
        StatValue.from_list(valuesPerCategory[result.CATEGORY_CORRECT,
                                              result.RESULT_CLASS_TRUE] +
                            valuesPerCategory[result.CATEGORY_CORRECT,
                                              result.RESULT_CLASS_FALSE]),
        StatValue.from_list(valuesPerCategory[result.CATEGORY_CORRECT,
                                              result.RESULT_CLASS_TRUE]),
        StatValue.from_list(valuesPerCategory[result.CATEGORY_CORRECT,
                                              result.RESULT_CLASS_FALSE]),
        StatValue.from_list(
            valuesPerCategory[result.CATEGORY_CORRECT_UNCONFIRMED,
                              result.RESULT_CLASS_TRUE] +
            valuesPerCategory[result.CATEGORY_CORRECT_UNCONFIRMED,
                              result.RESULT_CLASS_FALSE]),
        StatValue.from_list(valuesPerCategory[
        ...
        StatValue.from_list(
            valuesPerCategory[result.CATEGORY_WRONG, result.RESULT_CLASS_TRUE]),
        StatValue.from_list(valuesPerCategory[result.CATEGORY_WRONG,
                                              result.RESULT_CLASS_FALSE]),
    )


def get_regression_count(rows,
                         ignoreFlappingTimeouts):  # for options.dump_counts
    ...
    for row in rows:
        ...
        if (ignoreFlappingTimeouts and status_is(new, 'TIMEOUT') and
                any_status_is(row.results[:-2], 'TIMEOUT')):
...
def get_counts(rows):  # for options.dump_counts
    ...
    for runResults in rows_to_columns(rows):
        counts = collections.Counter(
            runResult.category for runResult in runResults)
        countsList.append((
            counts[result.CATEGORY_CORRECT],
            counts[result.CATEGORY_WRONG],
            counts[result.CATEGORY_UNKNOWN] + counts[result.CATEGORY_ERROR] +
...
def create_tables(name, runSetResults, rows, rowsDiff, outputPath,
                  outputFilePattern, options):
    '''
    ...
    common_prefix = os.path.commonprefix(
        [r.filename for r in rows])  # maybe with parts of filename
    ...
    firstBenchmarkName = Util.prettylist(
        runSetResults[0].attributes['benchmarkname'])
    ...
    template_values.count_id_columns = template_values.relevant_id_columns.count(
        True)
    ...
    def write_table(table_type, title, rows, use_local_summary):
        ...
        if not options.format == ['csv']:
            local_summary = get_summary(
                runSetResults) if use_local_summary else None
        ...
        for template_format in (options.format or TEMPLATE_FORMATS):
            if outputFilePattern == '-':
                outfile = None
                logging.info('Writing %s to stdout...',
                             template_format.upper().ljust(4))
            else:
                outfile = os.path.join(
                    outputPath,
                    outputFilePattern.format(
                        name=name, type=table_type, ext=template_format))
                logging.info('Writing %s into %s ...',
                             template_format.upper().ljust(4), outfile)
    ...
    if rowsDiff:
        write_table(
            "diff", name + " differences", rowsDiff, use_local_summary=False)

    return futures


def write_table_in_format(template_format, outfile, template_values,
                          show_table):
    # read template
    ...
    try:
        template_content = __loader__.get_data(template_file).decode(
            TEMPLATE_ENCODING)
    except NameError:
...
def create_argument_parser():
    ...
    parser.add_argument(
        ...
        dest="outputPath",
        help=
        "Output path for the tables. If '-', the tables are written to stdout.")
    parser.add_argument(
        "-n",
        "--name",
        action="store",
    ...
    parser.add_argument(
        ...
        dest="ignore_errors",
        help=
        "Ignore incomplete result files or results where the was an error during benchmarking."
    )
    parser.add_argument(
        "-d",
        "--dump",
        ...
        dest="dump_counts",
        help=
        "Print summary statistics for regressions and the good, bad, and unknown counts."
    )
    parser.add_argument(
        ...
        help=
        "For the regression-count statistics, do not count regressions to timeouts if the file already had timeouts before."
    )
    parser.add_argument(
        "-f",
        "--format",
        ...
        help=
        "Which format to generate (HTML or CSV). Can be specified multiple times. If not specified, all are generated."
    )
    parser.add_argument(
        "-c",
        "--common",
        ...
        dest="common",
        help=
        "Put only sourcefiles into the table for which all benchmarks contain results."
    )
    parser.add_argument(
        "--no-diff",
        action="store_false",
    ...
    parser.add_argument(
        ...
        dest="correct_only",
        help=
        "Clear all results (e.g., time) in cases where the result was not correct."
    )
    parser.add_argument(
        "--all-columns",
        action="store_true",
        dest="all_columns",
        help=
        "Show all columns in tables, including those that are normally hidden.")
    parser.add_argument(
        "--offline",
        action="store_const",
        dest="lib_url",
        ...
        default=LIB_URL,
        help=
        "Expect JS libs in libs/javascript/ instead of retrieving them from a CDN. "
...
def main(args=None):
    ...
    if options.xmltablefile:
        if options.tables:
            arg_parser.error("Invalid additional arguments '{}'.".format(
                " ".join(options.tables)))
        try:
            runSetResults = parse_table_definition_file(options.xmltablefile,
                                                        options)
        except Util.TableDefinitionError as e:
            logging.error('Fault in {}: {}'.format(options.xmltablefile,
                                                   e.message))
            exit(1)
        if not name:
        ...
        runSetResults = parallel.map(load_result, inputFiles,
                                     itertools.repeat(options))
        ...
        if len(inputFiles) == 1:
            ...
            if not name:
                name = NAME_START + "." + time.strftime("%Y-%m-%d_%H%M",
                                                        time.localtime())
    ...
    if not rows:
        ...
        exit()
    rowsDiff = filter_rows_with_differences(
        rows) if options.write_diff_table else []

    logging.info('Generating table...')
    if not os.path.isdir(outputPath) and not outputFilePattern == '-':
        os.makedirs(outputPath)
    futures = create_tables(name, runSetResults, rows, rowsDiff, outputPath,
                            outputFilePattern, options)

    if options.dump_counts:  # print some stats for Buildbot
        print("REGRESSIONS {}".format(
