...
REGEX_SIGNIFICANT_DIGITS = re.compile(
    '([-\+])?(\d+)\.?(0*(\d+))?([eE]([-\+])(\d+))?'
)  # compile regular expression only once for later uses
GROUP_SIGN = 1
GROUP_INT_PART = 2
GROUP_DEC_PART = 3
...
POSSIBLE_FORMAT_TARGETS = [
    'html', 'html_cell', 'tooltip', 'tooltip_stochastic', 'csv'
]
    ...
    * In addition, if 'unit' and 'source_unit' are set and of different values,
    ...
    def __str__(self):
        return "{}(title={}, pattern={}, num_of_digits={}, href={}, col_type={}, unit={}, scale_factor={})".format(
            self.__class__.__name__, self.title, self.pattern,
            self.number_of_significant_digits, self.href, self.type, self.unit,
            self.scale_factor)


def _format_number_align(formattedValue,
                         max_number_of_dec_digits,
                         format_target="html"):
...
def _format_number(number, initial_value_sig_digits,
                   number_of_significant_digits, max_digits_after_decimal,
                   isToAlign, format_target):
    """
    ...
    intended_digits = min(initial_value_sig_digits,
                          number_of_significant_digits)
    if number == 0:
        float_value = 0
    else:
        float_value = round(
            number, -int(math.floor(math.log10(number))) +
    ...
    if digits_to_add > 0:
        if '.' not in formatted_value:
            raise AssertionError(
                ...
                .format(formatted_value, number, float_value, intended_digits,
                        max_digits_to_display, format_target))
...
def _is_to_cut(value, format_target, is_to_align):
    correct_target = format_target == "html_cell" or (format_target == 'csv' and
                                                      is_to_align)
