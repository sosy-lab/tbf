...
def flatten(iterable, exclude=[]):
    return [
        value for sublist in iterable for value in sublist
...
def get_list_from_xml(elem, tag="option", attributes=["name"]):
    ...
    return flatten(
        ([option.get(attr)
          for attr in attributes] + [option.text]
...
def get_single_child_from_xml(elem, tag):
    ...
    if len(children) > 1:
        logging.warning(
            ...
            'ignoring all but the first.', elem.tag, tag)
    return children[0]
...
def parse_timespan_value(s):
    ...
    if not unit or unit == "s":
        ...
        raise ValueError(
            'unknown unit: {} (allowed are s, min, h, and d)'.format(unit))
...
def log_rmtree_error(func, arg, exc_info):
    ...
    logging.warning("Failure during '%s(%s)': %s", func.__name__, arg,
                    exc_info[1])
...
def shrink_text_file(filename, max_size, removal_marker=None):
    ...
    with open(filename, 'r+b') as output_file:
        with open(filename, 'rb') as input_file:
            ...
            )  # jump to end of current line so that we truncate at line boundaries
...
def _ProcessExitCode__str__(self):
    return (("exit signal " + str(self.signal))
            if self.signal else ("return value " + str(self.value)))
...
def add_files_to_git_repository(base_dir, files, description):
    ...
    gitAdd = subprocess.Popen(
        ['git', 'add', '--force', '--'] + files, cwd=gitRootDir)
    if gitAdd.wait() != 0:
