...
def _check_errno(result, func, arguments):
    ...
    if ((func.restype == c_int and result == -1) or
        (func.restype == c_void_p and
         c_void_p(result).value == c_void_p(-1).value)):
        errno = _ctypes.get_errno()
        try:
            func_name = func.__name__
        except AttributeError:
            func_name = "__unknown__"
        msg = func_name + "(" + ", ".join(map(
            str, arguments)) + ") failed: " + _os.strerror(errno)
...
clone.argtypes = [_ctypes.CFUNCTYPE(c_int), c_void_p, c_int,
                  c_void_p]  # fn, child_stack, flags, arg (varargs omitted)
...
mmap.argtypes = [c_void_p, c_size_t, c_int, c_int, c_int,
                 c_off_t]  # add, length, prot, flags, fd, offset
...
mount = _libc.mount
"""Mount a filesystem."""
mount.argtypes = [c_char_p, c_char_p, c_char_p, c_ulong,
                  c_void_p]  # source, target, fstype, mountflags, data
...
class CapData(_ctypes.Structure):
    ...
    _fields_ = ("effective", c_uint32), ("permitted", c_uint32), ("inheritable",
                                                                  c_uint32)
