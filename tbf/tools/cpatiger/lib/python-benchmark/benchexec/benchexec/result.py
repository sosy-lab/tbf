...
RESULT_LIST = [
    RESULT_TRUE_PROP, RESULT_UNKNOWN, RESULT_FALSE_REACH,
    _RESULT_FALSE_REACH_OLD, RESULT_FALSE_TERMINATION, RESULT_FALSE_DEREF,
    RESULT_FALSE_FREE, RESULT_FALSE_MEMTRACK, RESULT_WITNESS_CONFIRMED,
    RESULT_SAT, RESULT_UNSAT, RESULT_FALSE_OVERFLOW, RESULT_FALSE_DEADLOCK
]
...
_FILE_RESULTS = {
    ...
    '_true-valid-memsafety': (RESULT_TRUE_PROP,
...
def properties_of_file(propertyfile):
    ...
    if not ('CHECK' in content or content == 'OBSERVER AUTOMATON' or
            content == 'SATISFIABLE'):
        sys.exit(
            'File "{0}" is not a valid property file.'.format(propertyfile))
    ...
    if not properties:
        sys.exit('File "{0}" does not contain a known property.'.format(
            propertyfile))
    return properties
