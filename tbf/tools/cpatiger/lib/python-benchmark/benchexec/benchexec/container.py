...
""".format(
    host=socket.gethostname(), fqdn=socket.getfqdn())
...
def execute_in_namespace(func, use_network_ns=True):
    ...
    """
    flags = (signal.SIGCHLD | libc.CLONE_NEWNS | libc.CLONE_NEWUTS |
             libc.CLONE_NEWIPC | libc.CLONE_NEWUSER | libc.CLONE_NEWPID)
    if use_network_ns:
...
def setup_user_mapping(pid, uid=os.getuid(), gid=os.getgid()):
    ...
    try:
        uid_map = "{0} {1} 1".format(
            uid, os.getuid())  # map uid internally to our uid externally
    ...
    try:
        gid_map = "{0} {1} 1".format(
            gid, os.getgid())  # map gid internally to our gid externally
...
def activate_network_interface(iface):
    ...
    try:
        ...
        ifreq = struct.pack(STRUCT_IFREQ_LAYOUT_IFADDR_SAFAMILY, iface,
                            socket.AF_INET, b'0' * 14)
        ifreq = fcntl.ioctl(sock, SIOCGIFFLAGS, ifreq)
        if_flags = struct.unpack(STRUCT_IFREQ_LAYOUT_IFFLAGS, ifreq)[1]

        # Set new flags
        ifreq = struct.pack(STRUCT_IFREQ_LAYOUT_IFFLAGS, iface,
                            if_flags | IFF_UP, b'0' * 14)
...
def get_mount_points():
    ...
    with open("/proc/self/mounts", "rb") as mounts:
        for mount in mounts:
            source, target, fstype, options, unused1, unused2 = mount.split(
                b" ")
...
def make_overlay_mount(mount, lower, upper, work):
    logging.debug(
        "Creating overlay mount: target=%s, lower=%s, upper=%s, work=%s", mount,
        lower, upper, work)
    libc.mount(
        b"none", mount, b"overlay", 0,
...
def drop_capabilities():
    ...
    libc.capset(
        ctypes.byref(
            libc.CapHeader(version=libc.LINUX_CAPABILITY_VERSION_3, pid=0)),
...
def _forward_signal(signum, target_pid, process_name):
    ...
    try:
        ...
        logging.debug("Could not forward signal %d to process %s: %s", signum,
                      process_name, e)
...
def forward_all_signals_async(target_pid, process_name):
    ...
    def forwarding_signal_handler(signum):
        _forward_signal(signum, process_name,
                        forwarding_signal_handler.target_pid)
