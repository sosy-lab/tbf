...
def print_value(description, value, extra_line=False):
    print(
        '{}{}{}:{}“{}{}{}”'.format(
            COLOR_DESCRIPTION, description, COLOR_DEFAULT, '\n\t'
            if extra_line else ' ', COLOR_VALUE, value, COLOR_DEFAULT),
        file=sys.stderr)
...
def print_multiline_list(description, values):
    ...
    for value in values:
        print(
            '\t“{}{}{}”'.format(COLOR_VALUE, value, COLOR_DEFAULT),
            file=sys.stderr)
...
def analyze_tool_output(tool, file):
    ...
    try:
        ...
        logging.warning(
            'Tool module failed to analyze result in “%s”:',
            file.name,
            exc_info=1)
...
def main(argv=None):
    ...
    parser.add_argument(
        "tool", metavar="TOOL", help="name of tool info to test")
    parser.add_argument(
        "--tool-output",
        metavar="OUTPUT_FILE",
        nargs='+',
